import requests
from tkinter import Label, Button, Entry, StringVar, re, mainloop, ttk
...
class App(tk.Tk):
    ...
    def __init__(self, converter):
        ...
        # Label
        label_text = "Welcome to Real Time Currency Convertor"
        self.intro_label = Label(
            self, text=label_text,  fg='blue', relief=tk.RAISED, borderwidth=3)
        self.intro_label.config(font=('Courier', 15, 'bold'))

        self.date_label = Label(
            self, text=f"1 Euro equals = {self.currency_converter.convert('EUR', 'USD', 1)}"
        ...
        self.to_currency_dropdown = \
            ttk.Combobox(self, textvariable=self.to_currency_variable,
                         values=list(
                             self.currency_converter.currencies.keys()),
                         font=font,
                         state='readonly',
